   ...
   internal static class MapsterConfig
   {
      ...
      private static TypeAdapterConfig GetRvd145Config(this TypeAdapterConfig cfg)
      {
         cfg.NewConfig<Tuple<Rvd145, Device>, Rvd145Dto>()
            ...
            .Map(d => d.CoHighOutletTemp, s => s.Item1.CoHighOutletTemp)
            .Map(d => d.CoLowInletTemp, s => s.Item1.CoLowInletTemp)
            .Map(d => d.CoLowOutletPresure, s => s.Item1.CoLowOutletPresure)
            .Map(d => d.CoHeatCurveTemp, s => s.Item1.CoHeatCurveTemp)
            .Map(d => d.CoPumpStatus, s => s.Item1.CoPumpStatus)
            .Map(d => d.CoStatus, s => s.Item1.CoStatus)
         ...
         cfg.NewConfig<IEnumerable<Rvd145>, Rvd145ReportDto>()
            ...
            .Map(d => d.CoLowInletTempAvg, s => s.Average(x => x.CoLowInletTemp))
            .Map(d => d.CoLowInletTempMin, s => s.Min(x => x.CoLowInletTemp))
            .Map(d => d.CoLowInletTempMax, s => s.Max(x => x.CoLowInletTemp))
            .Map(d => d.CoLowOutletPresureAvg, s => s.Average(x => x.CoLowOutletPresure))
            .Map(d => d.CoLowOutletPresureMin, s => s.Min(x => x.CoLowOutletPresure))
            .Map(d => d.CoLowOutletPresureMax, s => s.Max(x => x.CoLowOutletPresure))
